...
describe('saveItemFactory works correctly', () => {
  const dummyList = (items: OrderedMap<Key, Item>): IState => ({
    list: {
      items: items,
      errors: {} as any,
  ...
  it('dispatch success on putItem resolves correctly two times', () => {
    ...
    const items = OrderedMap<Key, Item>({
      x: new Item({temporaryValue: 'XXX'}),
      y: new Item({temporaryValue: 'YYY'}),
    });

    const result = factory(['x', 'y'])(dispatchMock, () => dummyList(items), {});

    return result.then(() => {
      const savedCalled = dispatchMock.mock.calls
        .filter(x => x[0].type === actionTypes.ITEM_DELETE_SUCCESS);
  ...
  it('dispatch error on putItem resolves correctly two times', () => {
    ...
    return result.then(() => {
      const failedCalled = dispatchMock.mock.calls
        .filter(x => x[0].type === actionTypes.ITEM_DELETE_FAILED);
